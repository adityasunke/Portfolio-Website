...
import { csProjects } from '@/data/cs'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {csProjects.map((project, i) => (
            <AnimatedSection key={project.name} delay={i * 0.1}>
              <div className="group bg-surface border border-border rounded-xl p-6 h-full flex flex-col hover:border-cs-primary/40 hover:shadow-[0_0_28px_rgba(59,130,246,0.12)] transition-all duration-300">
                {/* Header */}
                <div className="flex items-start justify-between gap-2 mb-1">
                  <h3 className="text-lg font-heading font-semibold text-text-primary group-hover:text-cs-light transition-colors">
                    {project.name}
                  </h3>
                  {project.githubUrl && (
                    <a
                ...
                <ul className="space-y-1.5 mb-5 flex-1">
                  {project.bullets.map((bullet, bi) => (
                    <li key={bi} className="flex gap-2 text-xs text-text-secondary font-sans leading-relaxed">
                      <span className="text-cs-primary mt-1 shrink-0">▸</span>
                      {bullet}
                    </li>
                  ))}
                </ul>
                ...
                <div className="flex flex-wrap gap-1.5 pt-3 border-t border-border">
                  {project.techStack.map((tech) => (
                    <Badge key={tech} label={tech} variant="cs" size="sm" />
                  ))}
                </div>
              </div>
            </AnimatedSection>
